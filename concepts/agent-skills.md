...
`sources/dev/geeknews-2026-skill-file-management.md`의 경험담에서 정리한 실천안이며, 비교 실험으로 입증된 보편 규칙은 아니다.

1. 반복 업무와 실제 실패를 기준으로 필요한 스킬을 만든다.
2. Git 원본을 중심으로 공통·프로젝트 전용 스킬을 나누고 사본의 분화를 줄인다.
3. 상시 규칙·상세 절차·실행 코드를 구분한다. 문서 배치는 `concepts/context-engineering.md`를 참고한다.
4. 이전 대화가 없는 세션에서 실행 결과를 확인하고 실패를 회귀 사례로 남긴다. 모델·실행 환경이 바뀌면 다시 확인한다.
5. 공유본은 변경 검토 후 배포하고 불필요한 지침은 줄인다. 프로젝트 문서만으로 충분한 작업에는 스킬을 추가하지 않아도 된다.
...
- `sources/dev/geeknews-2026-skill-file-management.md` — 업무별 스킬의 버전 관리·배포·새 세션 검증 경험
