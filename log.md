...
- **이산 판사 → 이산 심판**: 같은 페이지 본문이 심판(judge/verifier)으로 20회 쓰는데 절 제목만 판사였다. 소스 `kwok-2026-llm-as-verifier` 도 함께 통일.
- **관문·검문소 → 게이트**: `loop-verification-gate` 는 한 문단 안에 게이트·관문·검문소 세 이름을 썼다. 발신 링크 3곳과 `index.md` 포함해 게이트로 통일.
- **기준자·천장 제거**: "평가 천장의 기준자"를 "평가의 상한"으로 바꾸고 왜 상한인지를 문장에 넣었다 — 바로 다음 문장이 같은 것을 상한이라 불러 이름이 갈려 있었다. `agentic-data-generation` 의 IAA 천장도 상한으로.
- **하네스 원어 병기**: 풀이가 `dynamic-workflows` 에만 있어, 그 페이지를 안 거친 독자는 대조할 원어가 없었다. `context-engineering`·`agentic-data-generation` 첫 등장에 추가.
- **통과 판정**: `kb-anchor-verification` 의 "닻(anchor)"은 첫 등장 원어 병기로 모범. 가드레일·게이트·해상도·심판은 소스에 근거가 있다 — **영어 단어로만 grep 하면 소스 페이지가 한국어라 없는 것처럼 보인다**(가드레일을 한 번 오판했다).
- 위 두 [2026-08-26]·[2026-08-25] 항목의 옛 표현은 당시 기록이라 손대지 않았다.
